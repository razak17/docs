...
local formatters = {
    ...
    go = function()
        print('No formatter installed for go.')
    end,
    c = function()
        print('No formatter installed for c.')
    end,
...
function M.format()
    ...
    local fn = formatters[ft] or function()
    end
    fn()
end
...
function M.setup()
    api.nvim_exec(
        "command! -nargs=0 Format :lua require'utils.formatter'.format()<CR>",
        "")
end
